get.proximal.genes <- function( enh.bed, gene.deseq.bed.sorted, distance.cutoff, closest.N, gene.pval.cutoff, up.down){
  ...
  if(!is.null(closest.N)){
    ...
    closest.tab <- read.table(pipe(get.closet.nth.command),  sep="\t", stringsAsFactors=FALSE)
    ...
    closest.tab <- read.table(pipe(get.proximal.command),  sep="\t", stringsAsFactors=FALSE)
