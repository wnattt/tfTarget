get.gene.bed <- function(file.gene, file.tres, step=1000){
    ...
    refGene <- read.table(file.gene, sep="\t")
    refGene <- refGene[ which(refGene$V5!=""),] 
    ...
    refGene     <-refGene[refGene[,1] %in% unique(read.table(file.tres,  sep="\t")[,1]),]
...
get.tres.bed <- function(file.tres){
    ...
    TREs    <- read.table(file.tres,  sep="\t")[,1:3]
...
dif.deseq2.gene <- function(files.query.plus, files.query.minus, files.control.plus, files.control.minus, gene.file, TRE.file, out.file, ncores=1){
    ...
    gene.tab <-read.table(pipe(paste( "LC_COLLATE=C sort -k1,1 -k2,2n", gene.deseq.bed)),  sep="\t")
